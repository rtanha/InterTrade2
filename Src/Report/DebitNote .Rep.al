report 50016 "Debit Note - Inter"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Src\Layout\Debit Note - Inter.rdlc';
    Caption = 'Debit Note';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            RequestFilterHeading = 'Debit Note';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(RequestedReceiptDate_PurchHeader; "Purchase Header"."Requested Receipt Date")
                    {
                    }
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(Purchase_Line___Unit_of_Measure_; UnitofMeasure.Description)
                        {
                        }
                        ...
                        column(Purch_line_Quantity_Base; "Purchase Line"."Quantity (Base)")
                        {
                            IncludeCaption = true;
                        }
                        column(PurchLine_QuantityBase_Caption; "Purchase Line".FieldCaption("Quantity (Base)"))
                        {
                        }
                        ...
                        column(UnitOfMeasureBase_PurchLine; UnitOfMeasureBase)
                        {
                        }
                        column(Vendor_Item_No_PurchLine; VendorItem)
                        {
                        }
                        column(CountryOfRegion_PurchLine; CountryOfOriginText)
                        {
                        }
                        ...
                        begin
                            ...
                            if Item.Get("Purchase Line"."No.") then begin
                                ...
                                DocMgt.TranslateUnitOfMeasure(UnitofMeasure, Item."Base Unit of Measure");
                                UnitOfMeasureBase := UnitofMeasure.Description;
                            end;
                            if "Purchase Line"."Unit of Measure Code" = '' then
                                UnitofMeasure.Init
                            else begin
                                UnitofMeasure.Get("Purchase Line"."Unit of Measure Code");
                                DocMgt.TranslateUnitOfMeasure(UnitofMeasure, "Purchase Line"."Unit of Measure Code");
                            end;
                            if "Purchase Line"."Vendor Item No." <> '' then
                                VendorItem := StrSubstNo('%1 %2', "Purchase Line".FieldCaption("Vendor Item No."), "Purchase Line"."Vendor Item No.")
                            else
                                VendorItem := '';
                            CountryOfOriginText := '';
                            if "Purchase Line"."Country of Origin (INT)" <> '' then begin
                                country.Get("Purchase Line"."Country of Origin (Int)");
    ...
    var
        ...
        CompanyInfo__Bank_Account_No__CaptionLbl: Label 'Registration No.:';
        Order_No_CaptionLbl: Label 'Debit Note No.';
        ...
        QuantityCaption: Label 'No. Package';
        UnitOfMeasureBase: Text[50];
        Item: Record Item;
        RequestedShippDate: Label 'Requested Receipt Date';
        VendorItem: Text;
